...
class Contest:
    ...
    def validate_questions(self, questions: List[Question]) -> None:
        for question in questions:
            if question.get_level() != self.get_level():
                raise RuntimeError(f"Question level doesn't match the contest level for questionId:: {question.get_id()}")
            
    def end_contest(self) -> None:
        self._status = ContestStatus.ENDED
